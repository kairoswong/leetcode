...
Generates:
  ...
  - site/scripts/solutions-data.js      (JS with index + code)
...
def scan_solutions():
    ...
    for fname in sorted(
        ...
        key=lambda f: int(f.split(".")[0]) if f.split(".")[0].isdigit() else 9999,
    ):
...
def _build_full(solutions):
    """Build full solution list with code."""
    full = []
    for s in solutions:
        entry = dict(s)
...
def write_index_js(solutions, output_path):
    """Write JS with index + code to site/scripts/solutions-data.js."""
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    # Build full list with code
    full = _build_full(solutions)
    with open(output_path, "w") as f:
        f.write("// Auto-generated by scripts/generate-index.py\n")
        f.write("// Full solution data with code.\n")
        f.write("const SOLUTIONS_DATA = ")
        json.dump(full, f, indent=2)
        f.write(";\n")
    print(
        f"✅ Wrote solutions-data.js ({len(solutions)} solutions with code) to {output_path}"
    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--json":
        ...
        write_json(solutions, OUTPUT_JSON)
        write_index_js(solutions, OUTPUT_INDEX_JS)
