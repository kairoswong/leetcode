#!/usr/bin/env python3
"""
generate-index.py — Scan solutions/ and generate data files.

Generates:
  - site/data/solutions-data.json       (JSON data for reference — full detail)
  - site/scripts/solutions-data.js      (JS with index + code + viz data)

Usage:
    python scripts/generate-index.py          # regenerate all data files
"""
...
SCRIPTS_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), 'site', 'scripts')
OUTPUT_JSON = os.path.join(DATA_DIR, 'solutions-data.json')
OUTPUT_INDEX_JS = os.path.join(SCRIPTS_DIR, 'solutions-data.js')
...
def write_json(solutions, output_path, viz_data):
    """Write solutions data to site/data/solutions-data.json (full detail)."""
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    full = []
    for s in solutions:
        entry = dict(s)
        entry['code'] = _extract_code(s['file'])
        entry['viz'] = viz_data.get(str(s['id']), None)
        full.append(entry)
...
def write_index_js(solutions, output_path, viz_data):
    """Write JS with index + code + viz data to site/scripts/solutions-data.js."""
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    # Build full list with code and viz
    full = []
    for s in solutions:
        entry = dict(s)
        entry['code'] = _extract_code(s['file'])
        ...
        full.append(entry)
    with open(output_path, 'w') as f:
        f.write('// Auto-generated by scripts/generate-index.py\n')
        f.write('// Full solution data with code + visualization.\n')
        f.write('const SOLUTIONS_DATA = ')
        json.dump(full, f, indent=2)
        f.write(';\n')
    print(f"✅ Wrote solutions-data.js ({len(solutions)} solutions with code + viz) to {output_path}")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        write_json(solutions, OUTPUT_JSON, viz_data)
        write_index_js(solutions, OUTPUT_INDEX_JS, viz_data)
